...
اسم مجلد القالب يبقى `magdi-hilal-adco` حتى لا تنكسر أحجام Docker. الاسم الظاهر للعلامة هو **HELAL CORP**.
...
## مستشار HELAL CORP (الدردشة)

زر ذهبي عائم يفتح لوحة **مستشار HELAL CORP** على كل الصفحات. يقبل العربية أو الإنجليزية، ويرد أساساً بالعربية (مع سطر إنجليزي قصير إن كان السؤال بالإنجليزية). زر واتساب لا يظهر ما دام لا يوجد رقم جوال.
...
2. في `.env` على الـ VPS: `WP_HOME` / `WP_SITEURL` / `WP_URL` = `https://helal.co` ثم أعد إنشاء الحاوية. إن بقي `magdyhelal.modevops.fun` في قاعدة البيانات، نفّذ `wp search-replace` كما في **[HOSTING.md](HOSTING.md)** وإلا رابط **الرئيسية** يكسر الموقع.
3. غيّر كلمة مرور `admin`
4. راجع الهاتف والبريد والعنوان في المخصص
5. ارفع شعارات العملاء الحقيقية وصور الفريق
