...
if ! wp core is-installed; then
  wp core install \
    --url="${WP_URL:-http://localhost:8088}" \
    --title="مكتب مجدي هلال — M.H CORP" \
    --admin_user="${WP_ADMIN_USER:-admin}" \
    --admin_password="${WP_ADMIN_PASSWORD:-admin}" \
    --admin_email="${WP_ADMIN_EMAIL:-magdy.hilal@co}" \
    --skip-email
fi
...
wp option update admin_email "${WP_ADMIN_EMAIL:-magdy.hilal@co}" || true
...
echo "Site:  ${WP_URL:-http://localhost:8088}"
echo "Admin: ${WP_URL:-http://localhost:8088}/wp-admin"
echo "User:  ${WP_ADMIN_USER:-admin}"
echo "Pass:  ${WP_ADMIN_PASSWORD:-admin}"
